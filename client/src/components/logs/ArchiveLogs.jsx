...
import { useLazyGetArchivedLogQuery, useLazyGetArchivedLogMinMaxDateQuery } from "@/services/LogServices";
...
const ArchivedLogs = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        ...
        await getMinMaxDate({ logName: logName, project: projectName });
      } catch (error) {
  ...
  if (isErrorDate) {
    toast.error(`${minMaxDate?.data?.message}` || "Failed to fetch logs");
  }
          ...
          <Button variant="outline" size="sm" className={"cursor-pointer"} onClick={() => setRefresh((prev) => !prev)}>
            <RefreshCcw
              className={cn("transition-transform", {
                "animate-spin": isLoading || isFetchingLogs || isErrorDate || isFetchingDate || isLoadingDate,
              })}
            />
          </Button>
